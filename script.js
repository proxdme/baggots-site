...
const heroImages = [
  {
    src: "images/red-zip-1.webp",
    alt: "Чёрная поясная сумка baggots с красными молниями"
  },
  {
    src: "images/yellow-line-1.webp",
    alt: "Чёрная поясная сумка baggots с жёлтыми молниями"
  },
  {
    src: "images/denim-pocket-1.webp",
    alt: "Джинсовая поясная сумка baggots"
  },
  {
    src: "images/coffee-run-1.webp",
    alt: "Поясная сумка baggots из кофейной упаковки"
  },
  {
    src: "images/london-mess-1.webp",
    alt: "Поясная сумка baggots с британским принтом"
  }
];
...
function renderHeroDots() {
  ...
  heroSliderDots.innerHTML = heroImages
    .map((_, index) => {
      return `<button class="hero-slider-dot ${index === 0 ? "active" : ""}" type="button" data-hero-index="${index}" aria-label="Показать фото ${index + 1}"></button>`;
    })
    .join("");
}

function setHeroImage(nextIndex) {
  if (!heroRotator || !heroImages[nextIndex]) return;

  heroRotator.classList.add("is-fading");

  setTimeout(() => {
    heroImageIndex = nextIndex;
    heroRotator.src = heroImages[heroImageIndex].src;
    heroRotator.alt = heroImages[heroImageIndex].alt;

    document.querySelectorAll(".hero-slider-dot").forEach((dot, index) => {
      dot.classList.toggle("active", index === heroImageIndex);
    });
...
function startHeroSlider() {
  if (!heroRotator || heroImages.length <= 1) return;
